...
setup_network() ->
    ...
    open_port({spawn, "erl -pa ../ebin -noshell -name  " ++ SecondNode ++ " -setcookie " ++ Cookie}, []),
    wait_until_running().

second_node() ->
    [Host, _] = re:split(os:cmd("ip.sh"), "\n"),
    binary_to_list(Host),
    "second_node@" ++ binary_to_list(Host).

secon_node_atom() ->
    list_to_atom(second_node()).

wait_until_running() ->
    case net_adm:ping(secon_node_atom()) of
        pong ->
            ok;
        pang ->
            timer:sleep(20),
...
close_network() ->
    rpc:call(secon_node_atom(), init, stop, []).
...
do_tests() ->
    ...
    do_tests({nodes, [{secon_node_atom(), 2}, secon_node_atom(),
        {secon_node_atom(), schedulers}]}),
    do_tests([{nodes, [{node(), 2}, secon_node_atom()]}, {timeout, 4000}, 4]),
