    ...
    {Best3, Others3, Sum3} =
        lists:foldl(
            fun(Node, {Best, Others, Sum}) ->
                Temp = if
                           Node#node.weight == 0 -> Node#node{weight = Node#node.cw};
                           true -> Node
                       end,
                Temp2 = Temp#node{cw = Temp#node.cw + Temp#node.weight},
...
test() ->
    performance_test:main(1000, 1000000, fun test_lb/1, [], 3).
