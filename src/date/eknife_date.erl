...
%%%
%%% @end
%%% Created : 01. 二月 2018 下午12:51
%%%-------------------------------------------------------------------
-module(eknife_date).
-export([beginning_of_month/1,
  date_to_string/1,
  date_to_string_new/1,
  date_to_bianry/1,
  day_of_week/1,
  easter/1,
  end_of_month/1,
...
date_to_string_new({Y, M, D}) ->
  true = calendar:valid_date({Y, M, D}),
  lists:flatten(io_lib:format("~4..0B~2..0B~2..0B", [Y, M, D])).

date_to_bianry({Y, M, D}) ->
  true = calendar:valid_date({Y, M, D}),
  DateString = lists:flatten(io_lib:format("~4..0B~2..0B~2..0B", [Y, M, D])),
  erlang:list_to_binary(DateString).
